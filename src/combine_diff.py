...
class Diff:
    ...
    def combine_diffs(self, start_version, end_version, page_id):
        ...
        while end_version > start_version:
            diff_file_path = os.path.join(PAGE_STORAGE, str(page_id) + "_" + str(end_version) + DIFF_SUFFIX)
    ...
    def generate_diffs(self, new_version, page_id):
        curr_file_name = os.path.join(PAGE_STORAGE, str(page_id))
        update_file_name = os.path.join(PAGE_STORAGE, str(page_id) + "_" + str(new_version))
        new_diff_file = os.path.join(PAGE_STORAGE, str(page_id) + "_" + str(new_version) + DIFF_SUFFIX)
        # Generate diffs
